...
The one exception (decided 2026-09-23, M29): a single code-created `UnreliableRemoteEvent`, server to client only, carrying packed `buffer` snapshots of monster positions (about 10 Hz, packets under 900 bytes). A lost snapshot is replaced by the next one, so reliability would only add cost. Every other monster event (spawn, death, HP, attacks) stays on the one RemoteEvent.

### Server state lives in Lua, not instances
Roblox replicates every server instance to clients at high cost with no throttle, so the server keeps a **minimal instance footprint**. World state is held in server-side Lua tables and validated there. For entities that need a world position, use the **shadow system**: one small invisible anchored Part per logical entity carrying attributes; clients watch the attributes and build all visuals locally (see `~/md/ghost_shadow.md` and `~/square/src/client/ClientShadowManager.luau`). The mine grid is Lua data sent to clients over the message bus, never parts on the server (M17). Shadows suit entities that are few or rarely move (posts, walls, towers, traps). Monsters are not shadowed (decided 2026-09-23, M29): with hundreds moving at once, per-part replication and attributes are too costly and streaming would pop them in and out. They are server Lua state stepped at a fixed rate, steered by one shared flow field, and sent as buffer snapshots; clients draw them from pooled Humanoid-free templates (`GameAssets/Monsters`, `scripts/studio/build_monster_templates.luau`) moved with `BulkMoveTo`, with distance LOD (the nearest few animated rigs, then the template's one-part baked `Held` union from `scripts/studio/bake_monster_held.luau` at every further distance, the `Proxy` block drawn only for a template without a `Held` yet; budgets per device tier in `GameData/Monsters/Behavior.DRAW_BUDGETS`, decided 2026-09-24), never culled.
...
### Maps (M17)
Every map is a Folder under `ServerStorage/Maps` named by its `Enums.Map` key; `MapManager` clones the run's map into Workspace as `Camp` at run start and `GameplayMapManager:checkCamp` validates it. To edit a map in Studio, run `scripts/studio/map_tools.luau` with `ACTION = "load"` (moves it into Workspace, pastes any terrain snapshot), then with `ACTION = "save"`. A map left in Workspace is used as-is with a warning. The mine's origin and facing come from the map's `Mine.Entrance` part; the rescue radio is `Camp.Radio` with `GuiPart`, `PromptPart` (its ProximityPrompt), and `Radio.Screen` (M31). Monsters walk the map's `WaveAreas` and `CampArea` parts and spawn at the numbered `MonsterSpawns` parts (`scripts/studio/build_monster_areas.luau`); marker parts are hidden at run start. The generated mine is never parts on the server: `MineManager` holds the grid and sends it as run-length-encoded buffer chunks with filler collapsed to a placeholder that the client re-rolls from the seed (`MineGenerator.expandFiller`); `ClientMineManager` renders only exposed blocks within the streaming radius from a part pool, cloning `GameAssets/Rocks` templates (`scripts/studio/build_rock_templates.luau`).
...
### Monsters (M29)
Everything a monster is lives in GameData: stats per `Enums.MonsterKind` in `GameData/Monsters/Roster`, engine and network tuning in `GameData/Monsters/Behavior`, per-map waves in `GameData/Waves/<Map>`. Its look and size live only on its template, `GameAssets/Monsters/<ENUM_KEY>`, whose invisible `Proxy` part is the hit box, the far-away drawing, and (bottom edge) the feet; `seed_monster_templates.luau` gives a new kind a starting copy. `MonsterPathManager` builds one grid and flow field from the areas (ground field recomputed when a barrier changes, air field once); `MonsterManager` steps every monster at `TICK_RATE_HZ`, registers each in `DamageableManager` without a shadow so every weapon and defense already works on it, and streams positions through `MonsterSnapshotCodec` on the one `UnreliableRemoteEvent`; `MonsterWaveManager` builds the night's wave (the authored one scaled to the crew, else pure `WaveGenerator` from the map's per-difficulty `GENERATOR` settings and `GameData/Waves/Generation`, M30; difficulty is parameters, never a stat multiplier), sets the crew's enemy HP multiplier, and clears the night; `ClientMonsterManager` draws from pools with distance LOD (rig, `Held` union, `Proxy`; touch and low graphics quality take the LOW draw budget), keeps samples as rings of plain numbers, moves only what changed, and never parents in and out; rerun `bake_monster_held.luau` after a monster's look changes. Pure rules (`FlowField`, `MonsterRules`, `MonsterSnapshotCodec`, `WaveRules`, `WaveGenerator`) are unit tested. The Monsters dev panel has spawn, kill all, the stress test with live server step and KB/s, and the path grid view.
...
- **Every GUI exists in StarterGui (or as a template under GameAssets/Gui for billboards and SurfaceGuis), never built in code** (decided 2026-09-21): a ScreenGui, a menu, an overlay, a HUD element, or a repeated element (a slot, a row, a flying icon) is authored as an instance the designer or an artist can select and adjust in the Explorer. Code only finds it by name, enables it, clones a template child, and sets values (text, image, colour, position). Code may create world adornments (Highlight, SelectionBox, ProximityPrompt) and effects, which are not GUI. Audit with a grep for `Instance.new("ScreenGui|Frame|TextLabel|TextButton|ImageLabel|BillboardGui")` in `src/`.
...
- **Resources and currency are icons, never words** (decided 2026-09-24): a count of Cash, Iron, Gold, Diamonds, Uranium, or ore shows the resource's icon from `Icons` beside the number ("[uranium] 12 / 20"), never "Uranium 12 / 20". A build script gives the row an `ImageLabel` with a `UIAspectRatioConstraint`; code sets the number only. Names of resources still appear in prose (a feedback line, a tooltip).
