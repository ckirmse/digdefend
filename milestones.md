...
Server holds the grid as Lua data, not parts. Send the grid to clients over the message bus in chunks; clients clone rock templates from `ReplicatedStorage/Rocks` and pool/stream blocks around the player. Rock HP tracked server-side by grid position. "Regenerate mine" dev tool with optional seed, so a designer can edit the mine file, sync, regenerate, and walk in without restarting the run. Measure bandwidth and memory against the 2 GB phone target.
