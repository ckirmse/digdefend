...
Run state: `Day` (5 minutes) to `Night` (spawning over 90 seconds, lasting until all enemies dead) to next `Day`. Day counter, clock HUD, gate auto open/close. Party scaling count sampled once at each dawn, only ever decreasing after the run starts; day-start message states the crew size and multipliers. Run ends on Refinery destroyed or all players dead.
