...
Pure-Lua generator implementing the ordered pipeline: resolve difficulty, resolve chamber inheritance, validate (fatal vs warn, naming parameter and chamber), size grid, fill bedrock, carve chambers with filler, stamp veins with corridor clearance, stamp hand-placed rock, carve corridors, place gates, report. Seeded and reproducible: the run seed is logged, and each chamber draws from its own sub-seed so editing one chamber never reshuffles another. The uranium fatal check runs on effective (post-difficulty) counts against the map's rescue target. Report includes requested vs placed ore per chamber and the share of diggable volume within pickaxe reach. Pure expected-ore preview (ore and HP totals from the mine file alone). Unit tests on the worked example from the GDD.
