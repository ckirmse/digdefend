...
Touching `JoinCollision` on an idle pad makes the toucher host: seated at the first `PlayerSpawn`, camera on `CameraPositionPart`, `CreateParty` menu opened (map carousel with locked maps desaturated plus requirement text, difficulty carousel re-evaluated per map, players stepper, Friends Only toggle, "Cancelling in N seconds", Create button disabled while the selection is locked). Creation timeout is a hard limit; expiry or leaving returns the host to `ExitSpawn` and the pad to idle. `TryCreateParty` carries the whole selection and is validated server-side against the host's completions (`CompletionKeys`), never the client's claim. On Create the pad boards: `BoardingTimeout` starts, host sees the Queue Menu in host mode (Launch button). Joiners are checked for pad state, room, not-already-in-a-party, and `IsFriendsWithAsync` for friends-only parties; they are not checked against unlocks. Full party drops the timer to `CapacityTime`; the timer never goes back up. Leaving via Exit, leaving the seat, or disconnecting; host leaving promotes the earliest joiner ("You are now the party host"), or idles the pad when empty. All feedback strings routed through `PlayerFeedbackManager`. Billboard rules (2026-09-10) kept: `Difficulty` background dark green for Normal, orange for Hard, red for Nightmare; label text from `Enums` public names.

**M13 — Launch and teleport**
Launch on host Launch press, boarding timeout, or capacity countdown; a party of one launches. Pad set to departing, helicopter takes off, departure loading screen fades in, characters moved to `HoldingArea` spawns, one `TeleportPartyAsync` to a fresh reserved gameplay server with map, difficulty, `playerCount` (actually teleported, never max), `hostUserId`, and `padIndex` in join data. Retry handling; failure respawns the player in `SpawnZone` with "Teleport failed"; a disconnect during departing is excluded. New helicopter cloned, pad cleared to idle. Gameplay server rejects players not in the reserved group.
